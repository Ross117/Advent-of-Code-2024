...
def find_summit_trails(row_ind: int, col_ind: int, trailhead_ind: int) -> None:
    ...
    for neightbour in neighbours:
        if in_map(neightbour[0], neightbour[1]):
            if map[neightbour[0]][neightbour[1]] - height == 1 and map[neightbour[0]][neightbour[1]] == 9:
                trailheads[trailhead_ind] += 1
            elif map[neightbour[0]][neightbour[1]] - height == 1:
                find_summit_trails(neightbour[0], neightbour[1], trailhead_ind)
...
for row_ind, row in enumerate(map):
    for col_ind, val in enumerate(row):
        if val == 0:
            ...
            find_summit_trails(row_ind, col_ind, trailhead_ind)
