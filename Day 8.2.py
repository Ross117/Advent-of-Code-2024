...
def find_antinodes(index_a: list[int], index_b: list[int]) -> list[list[int]]:
    ...
    # add the antenna
    antinodes.extend([index_a, index_b])

    original_row_diff: int = abs(index_a[0] - index_b[0])
    original_col_diff: int = abs(index_a[1] - index_b[1])

    row_diff = original_row_diff
    col_diff = original_col_diff

    while row_diff <= len(map) and col_diff <= len(map[0]):
